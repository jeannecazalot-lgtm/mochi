...
function FaceBtn({ face, on }) {
  ...
  return (
    <View style={[s.face, on ? { backgroundColor: onBg, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.hairline } : s.faceOff]}>
      <Text style={{ fontSize: 15, fontWeight: '600', color: colors.ink, opacity: on ? 1 : 0.55 }}>{face}</Text>
    </View>
  );
}
function VarianteB() {
    ...
    <Card padding={0} r={18}>
      <View style={{ paddingVertical: 14, paddingHorizontal: 14 }}>
        <View style={s.cLegend}>
          ...
          <Text style={s.cLegendTxt}><Text style={{ color: colors.sageDeep }}>● </Text>1 tap · j'aime</Text>
          <Text style={s.cLegendTxt}><Text style={{ color: colors.coralDeep }}>● </Text>2 taps · je déteste</Text>
        </View>
        <View style={s.cloud}>
