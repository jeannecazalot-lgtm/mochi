...
Côté Supabase : provider Apple **activé** (Client IDs = `app.annie.mochi`, pas de clé secrète : le natif iOS n'en a pas besoin, le dashboard refuse une clé vide seulement si on en tape une) ; modèles d'e-mail Magic Link + Change Email à passer en code `{{ .Token }}`. Sans ça : Apple échoue (« Connexion Apple impossible ») et l'e-mail envoie un lien au lieu d'un code.
