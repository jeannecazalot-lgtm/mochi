...
- **Décisions Jeanne (6 sept)** : (1) rejoignant qui a déjà rempli 06/07/08 → on saute ; (2) dépense = vraie table `expenses` synchronisée à deux ; (3) tâche créée via le + → à ma main (je propose : occurrences générées tout de suite) ; (4) Profil → dispos/préférences : à ma main ; (5) glissement sur les listes : à retrancher (question reformulée) ; tu/vous, largeur CTA, formats, écran 35, écran de connexion : à trancher plus tard.
