...
Côté Supabase (à finir) : provider Apple — Client IDs = `app.annie.mochi`, toggle activé, mais la sauvegarde ne semble pas prise (le dashboard rend mal dans Chrome) ; modèles d'e-mail Magic Link + Change Email à passer en code `{{ .Token }}`. Sans ça : Apple échoue (« Connexion Apple impossible ») et l'e-mail envoie un lien au lieu d'un code.
