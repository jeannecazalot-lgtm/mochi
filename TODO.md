...
**Synchro Supabase EN MARCHE (soir du 1er sept 2026, ~23h)** : « C'est parti » (12) crée le foyer et pousse membre (dispos + temps/sem), tâches, pénibilités et occurrences de la semaine — **vérifié en base** (6 tâches, 21 occurrences). Corrections en route : file offline sans upsert (`ON CONFLICT` refusé par la RLS avant d'être membre), ids de tâches stables entre synchros (rejouer ≠ dupliquer), `drain()` pour vider la file, **migration 0002 appliquée** (le créateur d'un foyer peut le lire avant d'en être membre — deadlock RLS). Reste en base : 2 foyers orphelins + quelques users anonymes de test à purger par Jeanne (je ne supprime pas de données) :
`delete from households where id not in (select household_id from household_members);`
`delete from auth.users where is_anonymous and id not in (select user_id from household_members);`
