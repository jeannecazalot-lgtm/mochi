...
- **Build 9** = tout ce qui précède, auto-soumis.
- À trancher ensemble (pas des bugs) : placement des tâches hebdo sans grille dispos (tout tombe aujourd'hui) · texte « Duo formé » côté rejoignant (« Bravo, X a rejoint » alors que c'est moi qui rejoins) · le hint « Glisser une mission… » sous une liste vide · départ volontaire d'un foyer (écran « quitter le foyer », requis pour rejoindre ailleurs quand on est déjà à deux).
