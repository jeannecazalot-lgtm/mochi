-- 0003 · Rejoindre un foyer quand on est SEUL dans le sien : on le quitte
-- automatiquement (le foyer orphelin et ses données disparaissent) puis on
-- rejoint. Corrige le blocage du test à deux du 2 sept 2026 : le foyer
-- naissant dès l'écran d'invitation (build 6), l'invitée avait déjà le sien
-- → accept_invitation refusait avec 'already_in_household' des deux côtés.
-- Refus conservés : déjà membre du foyer cible, ou foyer actuel à 2+ membres
-- (là, il faut une vraie sortie volontaire — écran « quitter le foyer »).
...
begin
  ...
  if my_hid is not null then
    ...
    -- seul dans mon foyer : je le quitte, il disparaît (cascade sur ses données)
